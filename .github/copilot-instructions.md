...
Supported selectors: `tag`, `.class`, `#id`, `*`, compounds (`h1.title`), comma-separated selector lists, and the descendant combinator (`nav a`, `#main .title`). Selectors are represented as `CssCompound { parts: [CssSimple] }` where the rightmost part is the subject; specificity is the sum of each part's specificity. Child (`>`), sibling (`+`/`~`), pseudo-classes, and attribute selectors are **not** matched — rules that contain them are parsed and skipped so they never "accidentally" match too broadly. `@media` queries: `(prefers-color-scheme: dark)` applies (OscaWeb is always-dark), `(prefers-color-scheme: light)` is dropped, and all other at-rule preludes (`print`, `min-width`, etc.) are dropped.

Supported properties: `color`, `background-color`/`background`, `font-weight`, `font-style`, `text-decoration`, `text-align` (`left`/`center`/`right`), `display: none`, `padding` (shorthand + T/R/B/L), `max-width` (px or %), `line-height` (unitless multiplier, px, or `normal`), and `margin: 0 auto` (sentinel for block centering — full margin box is not otherwise modelled). Inline `style=""` beats stylesheet rules; `!important` beats non-`!important`. Inheriting properties: `color`, `font-weight`, `font-style`, `text-decoration`, `text-align`, `line-height`. Non-inheriting: `padding`, `max-width`, `center_block`.
